...
import { bookingProperty, clearBookingStorage } from "@/src/Redux/actions/product.action";
import { clearUserStore } from "@/src/Redux/actions/user";
...
import { getVendorProductOrderedUser } from "@/src/Redux/actions/user";
import UpdateProductOrderStatus from "./OrderStatusUpdate";
...
function ViewProduct() {
    ...
    console.log(vendorCustomer, "<<<<<<<,,,,,")


    React.useEffect(() => {
        const fetchCustomerInfo = async () => {
            if (!currentUser) return
            if (callingType === "confirm") {
                const {
                    vendorRef,
                    _id
                } = route.params
                if (_id && vendorRef) {
                    await dispatch(getVendorProductOrderedUser(vendorRef, _id))
                }
            }
        }
        fetchCustomerInfo()
    ...
    //                     })
    //                     await dispatch(clearBookingStorage())
    ...
    const orderPlace = async () => {
        ...
        if (userInfo) {
            ...
            if (userData?._id && route.params?._id && route.params.vendorRef) {
                const data: BookingPayload = {
                    vendorRef: route.params.vendorRef,
                    customerRef: userData._id,
                    productRef: route.params._id,
    ...
    return (
        <SafeAreaView style={styles.root}>
            ...
            {
                callingType ==="confirm" && currentUser && vendorCustomer.status === "success" && vendorCustomer.data && Array.isArray(vendorCustomer.data) && (
                    <UpdateProductOrderStatus 
                       user={vendorCustomer.data}
